#ifndef NICEHERO_STATIC_VECTOR
#define NICEHERO_STATIC_VECTOR
#ifdef max
#undef max
#endif
/// \file
...
#define FCV_ASSERT(...)                                                       \
    static_cast<void>((__VA_ARGS__)                                           \
                          ? void(0)                                           \
                          : ::nicehero::fcv_detail::assert_failure(  \
...
//             FCV_EXPECT(static_cast<ptrdiff_t>(i) < (end(rng) - begin(rng)));
        ...
        namespace storage
        {
            ...
            struct zero_sized
            {
				...
				template <typename... Args,
					FCV_REQUIRES_(Constructible<T, Args...>)>
					static constexpr void emplace(size_t loc,Args&&...) noexcept
				{
					FCV_EXPECT(false
						&& "tried to emplace_back on empty storage");
				}
...
// 					FCV_REQUIRES_(Constructible<T, Args...>)>
        ...
        : private fcv_detail::storage::_t<T, Capacity>
    {
        ...
        constexpr reference at(size_type pos)
        {
            if (pos >= size())
            {
                throw out_of_range("static_vector::at");
            }
            return fcv_detail::index(*this, pos);
        }
        ...
        {
            FCV_EXPECT(!full()
                        && "tried insert on full static_vector!");
        ...
        constexpr static_vector&
        operator=(static_vector const& other) noexcept(
            noexcept(clear())
        ...
        constexpr static_vector(initializer_list<U> il) noexcept(
            noexcept(base_t(move(il))))
            : base_t(move(il))
        {  // assert happens in base_t constructor
        }
		constexpr static_vector(initializer_list<T> il)
		{ 
    ...
    constexpr bool operator==(
        static_vector<T, Capacity> const& a,
