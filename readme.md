...
**avl_array** is a C++ STL like container class to store data organzied as AVL tree in a **fixed size** array.

Motivation of this container is to randomly insert, update and find elements with a minimum of time.

## Highligths
- std::map like templated container class
- Statically allocated memory
- Highest speed, maximum performance and **no dependencies** (compared to std::map)
- Small memory overhead (just 2 * size_type per node)
...
Disadvantage is the storage overhead of three additional pointers for each node. If memory is an issue (like in embedded systems), this might be a problem.  
...
You can find a basic implementation of this method from Willem [here](https://github.com/willemt/array-avl-tree), but it seems to have several bugs and my test cases did not pass.  
...
Recursive function calls (which can be eliminated of course) are another problem.  
After some tests it became very clear that this method is weak when inserting new nodes on a big tree.  
Goal is to leave the nodes in place once they are inserted. This can only be achieved by changing pointer/index values.  
My implementation of the AVL tree class here is inspired by a [blog of Keith Woods](https://bitlush.com/blog/efficient-avl-tree-in-c-sharp) and his [implementation](https://github.com/bitlush/avl-tree-c-sharp) in C#. But instead of dynamic memory it uses a fixed array to store nodes and just two additional child indexes using the template given index type.  
So there's only a storage overhead of 2 * sizeof(size_type) * max_tree_size, e.g. 2 * 2 byte * 1023 = 4092 bytes for a 1023 node tree.  
